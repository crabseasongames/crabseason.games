...
function inventBug(config, canvas) {
  ...
  function modulateColor(i) {
    ...
    return `rgba(${config.color[0] * value}, ${config.color[1] * value}, ${config.color[2] * value}, 50)`;
  }
  ...
  addEventListener("click", (e) => {
    ...
    const bug = initializeBug(config.num_segments, config.arc_radius);
    const ctx = canvas.getContext("2d");
    moveBug(bug, newPosition);

    addEventListener("mousemove", (e) => {
      // moveBug(Bug, { x: e.clientX, y: e.clientY });
      newPosition = { x: e.clientX, y: e.clientY };
    });
    ...
    function initializeBug(N, R) {
      ...
      for (let i = 0; i < N; i++) {
        let x = newPosition.x;
        let y = newPosition.y + config.arc_radius * i;
